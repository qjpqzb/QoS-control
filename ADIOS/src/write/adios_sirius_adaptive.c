...
//#ifdef CEPH
		    ...
		    double ceph_start_time = MPI_Wtime ();
            ...
            rados_ioctx_t io_hdd;
		    char * hdd_poolname = "tier2_pool";
//#endif                    
                    
            ptr = fopen("/home/cephuser/reduced_data.bin","wb");
            ...
            /*
            fptr = fopen("/home/cephuser/full_data.bin","wb");
            ...
            */
            ...
            if (save_delta)
            {	
			            err = rados_ioctx_create(cluster, hdd_poolname, &io_hdd);
                        if (err < 0) {
                        	fprintf(stderr, "Cannot open rados pool %s: %s\n", hdd_poolname, strerror(-err));
                        ...
                        {
#ifdef WHOLE_BLOCK          
                            err = rados_write(io_hdd, "delta_L0_L1", delta, sizeof(double) * nelems, 0);
                            if (err < 0) {
                                	fprintf(stderr, "Cannot write object \"delta_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                	rados_ioctx_destroy(io_hdd);
                                	rados_shutdown(cluster);
                                	exit(1);
                            } else {
                                	printf("\nWrote Delta to object \"delta_L0_L1\".\n");
                            }

			    			err = rados_write(io_hdd, "delta_r_L0_L1", delta_r, sizeof(double) * nelems, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_r_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                                        exit(1);
                            } else {
                                        printf("\nWrote Delta to object \"delta_r_L0_L1\".\n");
                            }

			    			err = rados_write(io_hdd, "delta_z_L0_L1", delta_z, sizeof(double) * nelems, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_z_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                                        exit(1);
                            } else {
                                        printf("\nWrote Delta to object \"delta_z_L0_L1\".\n");
                            }
                            /*
                    		err = rados_write(io_hdd, "data",  (double*)data, sizeof(double) * nelems, 0);

							if (err < 0) {
										fprintf(stderr, "Cannot write object \"data\" to pool %s: %s\n",  hdd_poolname, strerror(-err));
										rados_ioctx_destroy(io_ssd);
										rados_shutdown(cluster);
										exit(1);
							} else {
										printf("\nWrote data to object \"data\" in pool %s.\n",hdd_poolname);
							}
							err = rados_write(io_hdd, "r", rr, sizeof(double) * nelems, 0);

							if (err < 0) {
										fprintf(stderr, "Cannot write object \"r\" to pool %s: %s\n",  hdd_poolname, strerror(-err));
										rados_ioctx_destroy(io_ssd);
										rados_shutdown(cluster);
										exit(1);
							} else {
										printf("\nWrote r data to object \"r\" in pool %s.\n",hdd_poolname);
							}

							err = rados_write(io_hdd, "z", zz, sizeof(double) * nelems, 0);

							if (err < 0) {
										fprintf(stderr, "Cannot write object \"z\" to pool %s: %s\n",  hdd_poolname, strerror(-err));
										rados_ioctx_destroy(io_ssd);
										rados_shutdown(cluster);
										exit(1);
							} else {
										printf("\nWrote r data to object \"z\" in pool %s.\n",hdd_poolname);
							}
                            */
...
//#ifdef CEPH
			                double ceph_end_time = MPI_Wtime ();
                            printf("ceph write io time = %f\n",ceph_end_time - ceph_start_time); 
//#endif 			    			    
                        ...
                        {  
                            ...
                            err = rados_write(io_hdd, "delta_L0_L1", delta_compr, compr_size_delta, 0);
                            if (err < 0) {
                                    fprintf(stderr, "Cannot write object \"delta_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                    rados_ioctx_destroy(io_hdd);
                                    rados_shutdown(cluster);
                            ...
                            err = rados_write(io_hdd, "delta_r_L0_L1", delta_r_compr, compr_size_delta_r, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_r_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                            ...
                            err = rados_write(io_hdd, "delta_z_L0_L1", delta_z_compr, compr_size_delta_z, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_z_L0_L1\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                            ...
                            err = rados_write(io_hdd, "delta_L0_L1_half", delta_half_compr, compr_size_delta_half, 0);
                            if (err < 0) {
                                    fprintf(stderr, "Cannot write object \"delta_L0_L1_half\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                    rados_ioctx_destroy(io_hdd);
                                    rados_shutdown(cluster);
                                    exit(1);
                            } else {
                                    printf("\nWrote Delta to object \"delta_L0_L1_half\".\n");
                            }

                            err = rados_write(io_hdd, "delta_r_L0_L1_half", delta_r_half_compr, compr_size_delta_r_half, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_r_L0_L1_half\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                                        exit(1);
                            } else {
                                        printf("\nWrote Delta to object \"delta_r_L0_L1_half\".\n");
                            }

                            err = rados_write(io_hdd, "delta_z_L0_L1_half", delta_z_half_compr, compr_size_delta_z_half, 0);
                            if (err < 0) {
                                        fprintf(stderr, "Cannot write object \"delta_z_L0_L1_half\" to pool %s: %s\n" , hdd_poolname, strerror(-err));
                                        rados_ioctx_destroy(io_hdd);
                                        rados_shutdown(cluster);
                                        exit(1);
                            } else {
                                        printf("\nWrote Delta to object \"delta_z_L0_L1_half\".\n");
                            }


                            err = rados_write(io_hdd, "data",  data_compr, compr_size_data, 0);
                            //err = rados_write_full(io_hdd, "data_o", data, 0);
                            ...
                            err = rados_write(io_hdd, "r", r_compr, compr_size_r, 0);
                            ...
                            err = rados_write(io_hdd, "z", z_compr, compr_size_z, 0);
                            ...
                            if (err < 0) {
                                        ...
                                        printf("\nWrote z data to object \"z\" in pool %s.\n",hdd_poolname);
                            }
							err = rados_write(io_hdd, "dpot_L1", data_reduced, sizeof(double)* nvertices_new, 0); 
            				err = rados_write(io_hdd, "r_L1", r_reduced, sizeof(double)* nvertices_new, 0);
            				err = rados_write(io_hdd, "z_L1", z_reduced, sizeof(double)* nvertices_new, 0);
                            ...
                            //printf("compr_size_data=%d\n",compr_size_data);
                            //err = rados_write(io_hdd, "r_o", rr, sizeof(double)*nelems, 0);
                            err = rados_write(io_hdd, "data_o", (double*) data, sizeof(double)*nelems,0);
                            err = rados_write(io_hdd, "r_o", rr, sizeof(double)*nelems,0);
                            err = rados_write(io_hdd, "z_o", zz, sizeof(double)*nelems,0);
                            //err = rados_write(io_hdd, "delta_L0_L1", delta_compr, compr_size_delta, 0);
                            //err = rados_write(io_hdd, "delta_r_L0_L1", delta_r_compr, compr_size_delta_r, 0);
                            //err = rados_write(io_hdd, "delta_z_L0_L1", delta_z_compr, compr_size_delta_z, 0);
                            
                            err = rados_write(io_hdd, "delta_L0_L1_qua", delta_qua_compr, compr_size_delta_qua, 0);
                                              
                            //if (err ==0) printf("Wrote delta_L0_L1_qua\n");
                            err = rados_write(io_hdd, "delta_L0_L1_o", delta, sizeof(double) * nelems, 0);
                            err = rados_write(io_hdd, "delta_r_L0_L1_o", delta_r, sizeof(double) * nelems, 0);
                            err = rados_write(io_hdd, "delta_z_L0_L1_o", delta_z, sizeof(double) * nelems, 0);
                            //printf("Finish wrotten to hdd osd\n");
